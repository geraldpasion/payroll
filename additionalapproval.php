...
if(isset($_POST['approved'])) { // if application is approved (if approve button was clicked)
	...
	if ($stmt = $mysqli->prepare("UPDATE others SET app_status = 'Approved', others_remarks = '$remarks', others_approvedby = '$approvedby', others_approvaldate='$approvaldate' WHERE others_id = '$othersid'")) {
		$stmt->execute();
		$stmt->close();
	...
	// insert the new record into the database
	if ($stmt = $mysqli->prepare("UPDATE others SET app_status = 'Disapproved', others_remarks = '$remarks', others_approvedby = '$approvedby', others_approvaldate='$approvaldate' WHERE others_id = '$othersid'")) {
		$stmt->execute();
		$stmt->close();
	} // show an error if the query has an error
	else {
