	...
	<script type="text/javascript" src="datepicker.js"></script> 
		...
		<script type="text/javascript">
		   	...
		   	function showList(){
		   		...
		   		document.getElementById('openEmp').style.display="block";
			}
		</script>
				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						...
						if(isset($_POST['keydate'])){
							...
							if(isset($_POST['test'])){
								...
								if ($result = $mysqli->query("SELECT DISTINCT employee_id, employee_firstname, employee_lastname FROM attendance INNER JOIN employee ON employee.employee_id = attendance.employee_id $keydate ORDER BY attendance_date")) //get records from db
								{
									if ($result->num_rows > 0) //display employee based on date range
									{
										...
										while ($row = $result->fetch_object())
										{
											...
											echo "<td></td>";
											echo "<td><button class='btn btn-info' type='button' name='open' onclick='showList()'><span class='glyphicon glyphicon-folder-open'>&nbsp;</span><span>Open</button></span></td>";
											echo "<td><a href='export_attendance.php'><button id='submit' type='submit' name='export' class='btn btn-primary'><span class='glyphicon glyphicon-file'>&nbsp;</span><span>Export Attendance</span></button></a></td>";
					...
					/*	if(isset($_POST['keydate'])){
							...
							if(isset($_POST['test'])){
								...
								echo "<a href='export_attendance.php'><button id='submit' type='submit' name='export' class='btn btn3 btn-w-m btn-primary' style='float:right;'><span class='glyphicon glyphicon-file'>&nbsp;</span><span>Export Attendace</span></button></a>
										...
										while ($row = $result->fetch_object())
										{
											...
											if($row->employee_type == "Fixed" || $row->employee_type == "Shifting"){
												if($breakout == "" || $breakin == "" || $timeout == ""){
													...
													// if($timein < $shiftArray[0] && $breakout > $shiftArray[0]){
													// 	$timein = $shiftArray[0];
													// 	$res1 = date('H:i', strtotime($breakout) - strtotime($timein) - strtotime('03:00'));
													// }else if($timein < $shiftArray[0] && $breakout < $shiftArray[0]){
													// 	$res1 = "00:00";
													// }else{
													// 	$res1 = date('H:i', strtotime($breakout) - strtotime($timein) - strtotime('03:00'));
													// }
													// 	$res1Array = array();
													// 	$res1Array = split(':', $res1);
													// 	$res1ArrayMin = $res1Array[1]/60;
													// 	$res1ArrayDec = sprintf('%.2f', $res1Array[0] + $res1ArrayMin);

													// if($breakin != "" || $timeout != ""){
													// 	$res2 = date('H:i', strtotime($timeout) - strtotime($breakin) - strtotime('03:00'));

													// 	$res2Array = array();
													// 	$res2Array = split(':', $res2);
													// 	$res2ArrayMin = $res2Array[1]/60;
													// 	$res2ArrayDec = sprintf('%.2f', $res2Array[0] + $res2ArrayMin);

													// 	$resTotal = sprintf('%.2f', $res1ArrayDec + $res2ArrayDec);
													// 	echo '<td>'.$resTotal.'</td>';
													// 	//$mysqli->query("UPDATE attendance SET attendance_hours='$resTotal' WHERE attendance_id='$attendance_id'");
													// }
											...
											// 		echo '<td>0.00</td>';
											// 		//$mysqli->query("UPDATE attendance SET attendance_hours='$zero' WHERE attendance_id='$attendance_id'");
											// 	}else if($timein < $ndStart && $breakout <= $ndStart || $breakin >= $ndEnd && $timeout > $ndEnd || $timein < $ndStart && $breakout > $ndStart || $breakin < $ndEnd && $timeout >= $ndEnd || $timein < $ndStart && $breakout < $ndEnd){
											...
											// 				$res1ArrayMin = $res1Array[1]/60;
											// 				$res1ArrayDec = sprintf('%.2f', $res1Array[0] + $res1ArrayMin);
											// 		}else if($timein < $ndStart && $timein >= $ndEnd){
											// 			if($timein > $breakout){
											...
											// 		if($breakin >= $ndEnd && $timeout > $ndEnd){
											// 			$res2 = date('H:i', strtotime($timeout) - strtotime($breakin) - strtotime('03:00'));
											...
											// }
												...
												// if($timein == ""){
												// 	echo '<td>0.00</td>';
												// }else if($timein >= $ndEnd && $breakout <= $ndStart || $timein >= $ndEnd && $breakout > $ndStart || $timein >= $ndStart && $breakout <= $ndEnd || $timein >= $ndStart && $breakout > $ndEnd || $timein < $ndStart && $breakout > $ndEnd){

												// 	if($timein >= $ndEnd && $breakout <= $ndStart){
												// 		$res1 = date('H:i', strtotime($breakout) - strtotime($timein) - strtotime('02:00'));
												// 	}else if($timein >= $ndEnd && $breakout > $ndStart){
												// 		$breakout = $ndStart;
												// 		$res1 = date('H:i', strtotime($breakout) - strtotime($timein) - strtotime('02:00'));
												// 	}else if($timein >= $ndStart && $breakout <= $ndEnd){
												// 		$res1 = "00:00";
												...
												// 		$res1 = date('H:i', strtotime($breakout) - strtotime($timein) - strtotime('02:00'));
												// 	}else if($timein < $ndStart && $breakout > $ndEnd){
												// 		$time1 = date('H:i', strtotime($ndStart) - strtotime($timein) - strtotime('02:00'));
												// 		$time2 = date('H:i', strtotime($breakout) - strtotime($ndEnd) - strtotime('02:00'));
												// 		$res1 = date('H:i', strtotime($time1) - strtotime($time2) - strtotime('02:00'));
												// 	}
