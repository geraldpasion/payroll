...
if($_GET['from'] == "one") {
	...
	$keydate = "WHERE attendance_date BETWEEN '$keydatefrom' AND '$keydateto' AND attendance.employee_id = '".$employee_id."' AND (attendance.status = 'Done' OR attendance.attendance_status = 'active' OR attendance.attendance_status = 'outforbreak' OR attendance.attendance_status = 'infrombreak')";
	if($attendace_result = $mysqli->query("SELECT * FROM employee INNER JOIN attendance ON employee.employee_id=attendance.employee_id $keydate ORDER BY attendance_date")){
		if($attendace_result->num_rows > 0){
			$empname = $mysqli->query("SELECT * FROM employee where employee_id='$employee_id'");
			...
			$output = '<table style="font-size: 12px;">
							<tr>
								<td><b>'.$fetch->employee_lastname.', '.$fetch->employee_firstname.' '.$fetch->employee_middlename.' ('. $fetch->employee_type .')</b></td>
							</tr>
							<tr>
								<td><b>Cut off: '. $keydatefrom .' to '.$keydateto.'</b></td>
							</tr>
							<tr>
								<td></td>
							</tr>
			...
			while($row = $attendace_result->fetch_object()){
				...
				$shiftArray = array();
				$shiftArray = split('-', $row->attendance_shift);
				$shift = date("g:i A", strtotime($shiftArray[0])).' - '.date("g:i A", strtotime($shiftArray[1]));
				$typeOfDay = $row->attendance_daytype;
				...
				$overbreak = sprintf('%.2f', $row->attendance_overbreak);
				...
				$REG_ND = sprintf('%.2f', $row->attendance_nightdiff);
	...
	if($emp_ids = $mysqli->query("SELECT DISTINCT employee.employee_id FROM employee INNER JOIN attendance ON employee.employee_id=attendance.employee_id $keydate ORDER BY employee.employee_id")){
		if($emp_ids->num_rows > 0){
			$output = '<table style="font-size: 12px;"><tr><td><b>DATE: '. $keydatefrom .' to '.$keydateto.'</b></td></tr><tr></tr><tr></tr>';
			while($employee = $emp_ids->fetch_object()) {
				$keydate = "WHERE attendance_date BETWEEN '$keydatefrom' AND '$keydateto' AND attendance.employee_id = '".$employee->employee_id."' AND (attendance.status = 'Done' OR attendance.attendance_status = 'active')";
				if($attendace_result = $mysqli->query("SELECT * FROM employee INNER JOIN attendance ON employee.employee_id=attendance.employee_id $keydate ORDER BY attendance_date")){
					if($attendace_result->num_rows > 0){
						$empname = $mysqli->query("SELECT * FROM employee where employee_id=".$employee->employee_id);
						$fetch = $empname->fetch_object();
						$shiftArray = array();
						$shiftArray = split('-', $fetch->employee_shift);
			...
			$output .= '</table>';
			header("Content-Type: application/xls");
	        header("Content-Disposition: attachment; filename=attendance".'_'."$datetoday.xls");
	        echo $output;
		}
	}
}// else if($_GET['from'] == "emp") {
//   	$employee_id = intval($_GET['id']);

// 	$datetoday = date('Y-m-d');

// 	$keydate = "WHERE attendance.employee_id = '".$employee_id."' AND (attendance.status = 'Done' OR attendance.attendance_status = 'active' OR attendance.attendance_status = 'outforbreak' OR attendance.attendance_status = 'infrombreak')";
// 	if($attendace_result = $mysqli->query("SELECT * FROM employee INNER JOIN attendance ON employee.employee_id=attendance.employee_id $keydate ORDER BY attendance_date")){
// 		if($attendace_result->num_rows > 0){
// 			$empname = $mysqli->query("SELECT * FROM employee where employee_id='$employee_id'");
// 			$fetch = $empname->fetch_object();
