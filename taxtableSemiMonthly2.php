...
function TaxSemiMonthly2($TaxCode, $NetTaxableIncome){
  ...
  //find right values
	for($i=7; $i>0; $i--){
		...
		if($NetTaxableIncome>=$TaxValues[$TaxCode][$Level]['GrossCheck']){
			...
			//echo $Level.'<br>';
			//echo 'GrossCheck: '.$TaxValues[$TaxCode][$Level]['GrossCheck'].'<br>';
			...
			$PercentedRemainingGross = $GrossMinusFixedTax * ($PercentOver/100);
			//echo 'PercentedRemainingGross: '.$PercentedRemainingGross.'<br>';

			$NetIncomeAfterTax = $NetTaxableIncome - $FixedTaxAmount - $PercentedRemainingGross;
